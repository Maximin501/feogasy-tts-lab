...
Point d'entrée de la semi-automatisation de la grille d'écoute VoxCPM2
(Feogasy · Binôme TTS - Données & Évaluation).

Ce module orchestre le déroulé complet :
  1. lit les arguments de la ligne de commande ;
  2. construit la liste des échantillons à évaluer (via grille.py) ;
  3. exclut ceux déjà évalués (reprise de session) ;
  4. boucle sur les échantillons restants : contrôle technique + lecture
     audio (via audio_io.py), saisie de la grille (via grille.py),
     sauvegarde immédiate ligne par ligne ;
  5. gère l'interruption (Ctrl+C) proprement, sans perte de données.

Toute la logique métier (structures, questions, CSV) vit dans grille.py.
Toute l'E/S audio bas niveau vit dans audio_io.py.
Les trois fichiers doivent rester dans le même dossier.

Exemples d'utilisation :
  # Sans aucun argument : le script demande l'évaluateur·rice de façon interactive
  # et utilise les chemins par défaut (eval/reference_samples, eval/reports/resultats.csv).
  # C'est le mode à utiliser avec le bouton "Run" de PyCharm.
  python main.py

  # Un seul checkpoint, pas de mode aveugle
  python main.py \\
...
Format attendu du manifeste (CSV, séparateur virgule, encodage UTF-8) :
  fichier,categorie,texte
  001.wav,Français,"Bonjour, comment allez-vous ?"
  002.wav,Malgache,"Manao ahoana ianao ?"
  ...
  (les colonnes 'categorie' et 'texte' sont optionnelles mais recommandées)

Dépendances :
  - stdlib uniquement pour le cœur du programme
  - optionnel : `simpleaudio` pour une lecture audio bloquante propre en Python pur
    (sinon, repli automatique sur le lecteur système : afplay / aplay / paplay / PowerShell)
  - optionnel : `numpy` pour les contrôles de niveau/silence (sinon ces contrôles sont
    simplement ignorés avec un avertissement, le reste du programme fonctionne quand même)
"""
